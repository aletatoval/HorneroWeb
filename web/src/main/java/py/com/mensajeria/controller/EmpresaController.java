...
import static py.com.mensajeria.utils.utils.Utils.generarClaveAleatoria;
...
import py.com.mensajeria.model.ejb.EmpresaManager;
	...
	public HashMap<String, Object> agregarEmpresa(@RequestBody Empresa nuevo,  HttpServletResponse response) {
		...
		try {
			...
			if (nuevo.getAlias() == null
				...
				throw new ExceptionMensajeria("alias","245",
			...
			if (empresaManager.total(ejemploAlias) > 0) {
				throw new ExceptionMensajeria("alias","246", "El alias "
			...
			if (nuevo.getRuc() == null
				...
				throw new ExceptionMensajeria("ruc","247",
			...
			if (empresaManager.total(ejemploRuc) > 0) {
				throw new ExceptionMensajeria("ruc","248", "El RUC " + nuevo.getRuc()
		...
		try {
			...
			if (empresaBD != null
					...
					&& !(Long.parseLong(empresaBD.get("id").toString()) == anterior
							.getId())) {
				throw new ExceptionMensajeria("nombre","300", "La empresa con el nombre "
			...
			if (empresaAlias != null
					...
					&& !(Long.parseLong(empresaAlias.get("id").toString()) == anterior
							.getId())) {
				throw new ExceptionMensajeria("alias","300", "El alias "
			...
			if (empresaRuc != null
					...
					&& !(Long.parseLong(empresaRuc.get("id").toString()) == anterior
							.getId())) {
				throw new ExceptionMensajeria("ruc","300", "El ruc "
